...
RSpec.describe Company, type: :model do
  ...
  it 'Expects to be invalid if email is not present' do
    @company1.email = nil
    @company1.save
    expect(@company1.errors.full_messages).to eq(["Email can't be blank", "Email is too short (minimum is 9 characters)"])    
  end

  it 'Expects to be invalid if password is not present' do
    @company1.password = nil
    @company1.save
    expect(@company1.errors.full_messages).to eq(["Password can't be blank", "Password is too short (minimum is 4 characters)"])    
  end
